...
#include "imx458mipiraw_Sensor.h"

/**********************Modify Following Strings for Debug**********************/
#define PFX "IMX458_camera_sensor"
#define LOG_1 LOG_INF("IMX458,MIPI 4LANE\n")
/***********************   Modify end    **************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	/* IMX458MIPI_SENSOR_ID, sensor_id = 0x2680*/
	.sensor_id = IMX458_SENSOR_ID,
...
#define IMX458_RAW_TYPE	(0x80)	/*enable test mode */

#define IMX458_HDR_TYPE (0x00)
#define IMX458_BINNING_TYPE (0x10)
#define IMX458_BW_TYPE		(0x20)
#define IMX458_NOPDAF_TYPE	(0x30)
#define IMX458_HDD_TYPE (0x40)	/* IMX458-0AUH5 */
static kal_uint16 imx458_type;
static kal_uint16 test_Pmode;
/*
 *-IMX458 0AQH5-C (BME-HDR version ,PDAF?¢Gsupport binning mode)
 *-IMX458 0APH5-C (Binning version ,HDR?¢Gsupport PDAF)
 *-IMX458 0AMH5-C (B/W version)
 *-IMX458 0ATH5-C (Non-PDAF version ,|3HDR)
 *-IMX458 0AUH5-C (Horizontal Double Density version)
 */
/* Sensor output window information */
/*according toIMX458 datasheet p53 image cropping*/
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	...
	{4208, 3120, 8, 600, 4192, 1920, 1048, 480,
...
#define IMX458MIPI_MaxGainIndex (154)
kal_uint16 IMX458MIPI_sensorGainMapping[IMX458MIPI_MaxGainIndex][2] = {
	{64, 0},
	{65, 8},
	{66, 16},
	{67, 25},
...
static kal_uint8 IMX458MIPI_SPC_Data[126];
static kal_uint8 SPC_data_done;
static void load_imx458_SPC_Data(void)
{
	if (SPC_data_done == false) {
		if (!read_imx458_eeprom_SPC(0x0F73, IMX458MIPI_SPC_Data, 126)) {
			LOG_INF("imx458 load spc fail\n");
			return;
		}
		SPC_data_done = true;
	}
...
static void write_imx458_SPC_Data(void)
{
	kal_uint16 i;
	...
	for (i = 0; i < 63; i++) {
		write_cmos_sensor(0xD04C + i, IMX458MIPI_SPC_Data[i]);
		/* LOG_INF("SPC_Data[%d] = %d\n", i, */
		/*	IMX458MIPI_SPC_Data[i]); */
	}
	for (i = 0; i < 63; i++) {
		write_cmos_sensor(0xD08C + i, IMX458MIPI_SPC_Data[i + 63]);
		/* LOG_INF("SPC_Data[%d] = %d\n", i+63, */
		/*	IMX458MIPI_SPC_Data[i+63]); */
	}
}
	...
	if (write_cmos_sensor(0x0A02, 0x0F) == 0) {
		...
		while (retry--) {
			if (read_cmos_sensor(0x0A01) == 0x01) {
				imx458_type = read_cmos_sensor(0x0A2E);

				if (imx458_type == IMX458_HDD_TYPE) {
					if (test_Pmode)
						imx458_type = IMX458_RAW_TYPE;
					else
						imx458_type = IMX458_HDD_TYPE;
				}

				LOG_INF(
				    "imx458 type = 0x%x(0x00=HDR,0x10=binning,0x40=HDD,0x80=RAW)",
				    imx458_type);
				return (kal_uint16) (
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (i = 0; i < IMX458MIPI_MaxGainIndex; i++) {
		if (gain <= IMX458MIPI_sensorGainMapping[i][0])
			break;
	}
	if (gain != IMX458MIPI_sensorGainMapping[i][0])
		LOG_INF("Gain mapping don't correctly:%d %d\n", gain,
			IMX458MIPI_sensorGainMapping[i][0]);
	return IMX458MIPI_sensorGainMapping[i][1];
}
...
/*No Need to implement this function*/
	...
	/*      preview_setting  */
static void imx458_ImageQuality_Setting(void)
{
	if (imx458_type == IMX458_HDR_TYPE || imx458_type == IMX458_RAW_TYPE) {
	...
	} else if (imx458_type == IMX458_BINNING_TYPE) {
		write_cmos_sensor(0x94C7, 0xFF);
		write_cmos_sensor(0x94C8, 0xFF);
		write_cmos_sensor(0x94C9, 0xFF);
		write_cmos_sensor(0x95C7, 0xFF);
...
static void sensor_init(void)
{
	...
	imx458_ImageQuality_Setting();
	/*Need Mirror/Flip */
	set_mirror_flip(0);

	write_imx458_SPC_Data();
...
static void capture_setting(kal_uint16 curretfps, kal_uint8 pdaf_mode)
{
	...
	if (imx458_type == IMX458_HDR_TYPE || imx458_type == IMX458_RAW_TYPE) {
		...
		write_cmos_sensor(0x7BCD, 0x00);
	} else if (imx458_type == IMX458_HDD_TYPE) {
		write_cmos_sensor(0x94DC, 0x20);
		write_cmos_sensor(0x94DD, 0x20);
		write_cmos_sensor(0x94DE, 0x20);
		write_cmos_sensor(0x95DC, 0x20);
	...
	if (imx458_type == IMX458_HDR_TYPE || imx458_type == IMX458_RAW_TYPE) {
		...
		write_cmos_sensor(0x7BCD, 0x00);
		if (imx458_type == IMX458_HDR_TYPE ||
			imx458_type == IMX458_RAW_TYPE) {
			write_cmos_sensor(0x94DC, 0x20);
			write_cmos_sensor(0x94DD, 0x20);
			write_cmos_sensor(0x94DE, 0x20);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	s_DEVINFO_ccm->device_ic = "IMX458";
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imx458_type == IMX458_HDR_TYPE)
		...
		sensor_info->PDAF_Support = PDAF_SUPPORT_CAMSV;
	else if (imx458_type == IMX458_RAW_TYPE)
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		...
		read_imx458_pdaf((kal_uint16) (*feature_data),
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			if (imx458_type != IMX458_BINNING_TYPE)
				memcpy(
				(void *)PDAFinfo,
	...
	case SENSOR_FEATURE_GET_PDAF_TYPE:
		...
		case 0x00:
			sprintf(
			      feature_para, "IMX458 0AQH5-C (BME-HDR version)");
			break;

		case 0x10:
			sprintf(
			      feature_para, "IMX458 0APH5-C (Binning version)");
			break;

		case 0x20:
			sprintf(
				feature_para, "IMX458 0AMH5-C (B/W version)");
			break;

		case 0x30:
			sprintf(
			     feature_para, "IMX458 0ATH5-C (Non-PDAF version)");
			break;
		...
		case 0x40:
			...
			break;
		case 0x80:
			sprintf(feature_para, "IMX458 enable PDO");
			break;
		default:
			sprintf(feature_para, "Other case %x", imx458_type);
		}
...
/*UINT32 IMX458_MIPI_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)*/
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      IMX458_MIPI_RAW_SensorInit      */
