...
#define PFX "S5KGM1STGMS_STEREO_OTP"
#undef LOG_INF
...
int write_dualcam_cali_flag_file_s5kgm1stgms(void)
{
...
int write_dualcam_cali_result_flag_file_s5kgm1stgms(int result)
{
...
#if 0
int dump_dualcam_cali_flag_file_s5kgm1stgms(void)
{
    struct file *eeprom_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int dump_dualcam_cali_result_flag_file_s5kgm1stgms(int result)
{
    struct file *result_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int store_dualcam_cali_data_s5kgm1stgms(void)
{
    int size = -1, rc = -1;
    BYTE checkSum = 0;
	int ret = 1;
    ...
    rc = write_dualcam_cali_flag_file_s5kgm1stgms();
    if (rc < 0) {
    ...
    rc = write_dualcam_cali_result_flag_file_s5kgm1stgms(ret);
    if (rc < 0) {
...
int dump_dualcam_cali_data_s5kgm1stgms(void)
{
    struct file *eeprom_file = NULL;
...
#if 0
    err = dump_dualcam_cali_flag_file_s5kgm1stgms();
    ...
    err = dump_dualcam_cali_result_flag_file_s5kgm1stgms(ret2);
    if (err < 0) {
        printk("%s: failed to write_dualcam_cali_flag_file\n", __func__);
        //return -1;
        ret2 = -1;
