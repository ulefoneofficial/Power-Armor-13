...
static int aw881xx_on_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static int aw881xx_on_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct aw881xx *aw881xx = snd_soc_codec_get_drvdata(codec);
//	struct device_ops *dev_ops = &(aw881xx->dev_ops);
    ...
    pr_info("aw881xx_on_set");


	if (!(aw881xx->flags & AW881XX_FLAG_START_ON_MUTE)){
		pr_info("aw881xx_on_set return at line 685");
		return 0;
...
static int aw881xx_off_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
static int aw881xx_off_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct aw881xx *aw881xx = snd_soc_codec_get_drvdata(codec);
    ...
    pr_info("aw881xx_off_set");


	if (!(aw881xx->flags & AW881XX_FLAG_START_ON_MUTE)){
		pr_info("aw881xx_off_set return at line 722");
		return 0;
	}
...
static struct snd_kcontrol_new aw881xx_controls[] = {
	...
	SOC_ENUM_EXT("aw881xx_startup", aw881xx_snd_enum[0],
		aw881xx_on_get, aw881xx_on_set),

	SOC_ENUM_EXT("aw881xx_shutdown", aw881xx_snd_enum[0],
		aw881xx_off_get, aw881xx_off_set),
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		pr_err("%s: requested rate: %d, sample size: %d\n",
...
//	struct aw881xx *aw881xx = snd_soc_codec_get_drvdata(codec);
	...
	pr_info("%s: mute state=%d do nothing \n", __func__, mute);
	...
	unsigned int reg, unsigned int value)
{
	...
	if (dev_ops->get_reg_access(reg) & REG_WR_ACCESS)
		ret = aw881xx_reg_write(aw881xx, (uint8_t)reg, (uint16_t)value);
	else
...
int aw881xx_update_cfg_name(struct aw881xx *aw881xx)
{
	...
	for (i = 0; i < sizeof(aw881xx_cfg_name)/AW881XX_CFG_NAME_MAX; i++)
		if (i % AW881XX_MODE_CFG_NUM_MAX)
			memcpy(aw881xx_cfg_name[i] + head_index, buf, sizeof(buf) - 1);
...
int aw881xx_read_dsp_pid(struct aw881xx *aw881xx)
{
	...
	case AW881XX_DSP_PID_03:
		...
		break;
	default:
		pr_info("%s: unsupported dsp_pid=0x%04x\n",
			__func__, reg_val);
		return -EIO;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	aw881xx_reg_read(aw881xx, dsp_mem_data_reg, &reg_val);
	len += snprintf(buf + len, PAGE_SIZE - len,
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (2 == sscanf(buf, "%x %x", &databuf[0], &databuf[1])) {
		...
		aw881xx_reg_write(aw881xx, dsp_mem_addr_reg, databuf[0]);
		aw881xx_reg_write(aw881xx, dsp_mem_data_reg, databuf[1]);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (aw881xx->dsp_cfg == AW881XX_DSP_BYPASS) {
		...
		if (ret < 0) {
			...
			aw881xx_reg_write(aw881xx, dsp_mem_addr_reg,
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	aw881xx->cali_re = databuf[0] * (1 << AW881XX_DSP_RE_SHIFT) / 1000;
	aw881xx_set_cali_re(aw881xx);
