 ...
 * Setting version:
 * ------------
 *   update full pd setting for OS12D40EB_03B
 *------------------------------------------------------------------------------
 * Upper this line, this part is controlled by CC/CQ. DO NOT MODIFY!!
 *============================================================================
 ****************************************************************************/

#define PFX "OS12D40_camera_sensor"
...
#include "os12d40_mipiraw_Sensor.h"
#include "os12d40_Sensor_setting.h"
...
#if 0
#define DEVICE_VERSION_OV12D    "OS12D40"
static uint8_t deviceInfo_register_value;
#endif
static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OS12D40_SENSOR_ID,
    .module_id = OS12D40_SENSOR_ID,
	...
	.max_frame_length = 0x7FEF,     /* max framelength by sensor register's limitation */
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_4CELL_HW_BAYER_R,   //SENSOR_OUTPUT_FORMAT_RAW_4CELL_HW_BAYER_B  prize--huangjiwu--modify
	.mclk = 24,//mclk value, suggest 24 or 26 for 24Mhz or 26Mhz
	.mipi_lane_num = SENSOR_MIPI_4_LANE,//mipi lane num
	.i2c_addr_table = {0X20, 0x6c, 0xff},  // 0V48B2Q 0X20   OV12D  0X6D   //0x20
	.i2c_speed = 400,
};
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	{4608, 2592,    0,    0, 4608, 2592, 2304, 1296,  0,  0, 2304, 1296, 0, 0, 2304, 1296},     /* Preview check*/
	{4608, 2592,    0,    0, 4608, 2592, 4608, 2592,  0,  0, 4608, 2592, 0, 0, 4608, 2592},      /* capture */
	{4608, 2592,    0,    0, 4608, 2592, 2304, 1296,  0,  0, 2304, 1296, 0, 0, 2304, 1296},     /* video */
	{4608, 2592,    0,    0, 4608, 2592, 2304, 1296,  0,  0, 2304, 1296, 0, 0, 2304, 1296},      /* hs vedio */
	{4608, 2592,    0,    0, 4608, 2592, 2304, 1296,  0,  0, 2304, 1296, 0, 0, 2304, 1296},     /* slim vedio */
};
...
static kal_uint16 OS12D40_table_write_cmos_sensor(
...
static void sensor_init(void)
{
	...
	OS12D40_table_write_cmos_sensor(
		addr_data_pair_init_OS12D40,
		sizeof(addr_data_pair_init_OS12D40) / sizeof(kal_uint16));
...
static void preview_setting(void)
{
	...
	OS12D40_table_write_cmos_sensor(
		addr_data_pair_preview_OS12D40,
		sizeof(addr_data_pair_preview_OS12D40) / sizeof(kal_uint16));
}
...
static void capture_setting(kal_uint16 currefps)
{
	...
	OS12D40_table_write_cmos_sensor(
		addr_data_pair_capture_OS12D40,
		sizeof(addr_data_pair_capture_OS12D40) /
		sizeof(kal_uint16));
}
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	OS12D40_table_write_cmos_sensor(
		addr_data_pair_video_OS12D40,
		sizeof(addr_data_pair_video_OS12D40) /
		sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	...
	OS12D40_table_write_cmos_sensor(
		addr_data_pair_hs_video_OS12D40,
		sizeof(addr_data_pair_hs_video_OS12D40) /
		sizeof(kal_uint16));
}
...
static void slim_video_setting(void)
{
	...
	OS12D40_table_write_cmos_sensor(
		addr_data_pair_slim_video_OS12D40,
		sizeof(addr_data_pair_slim_video_OS12D40) /
		sizeof(kal_uint16));
}
...
static kal_uint8 gOV12D_SN[CAMERA_MODULE_SN_LENGTH];
static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gOV12D_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		...
		LOG_INF("OV12D_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gOV12D_SN[idx]);
	}

	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(OV12D_STEREO_START_ADDR_WIDE+idx)>>8,
		(char)((OV12D_STEREO_START_ADDR_WIDE + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&gOtpCheckdata[0][idx],1, 0xA0);
		LOG_INF("OV12D_info[%d]: 0x%x\n", idx, get_byte[idx]);
	}

	for (idx = 0; idx <40; idx++) {
		char pusendcmd[2] = {(OV12D_STEREO_START_ADDR_TELE+idx)>>8,
		(char)((OV12D_STEREO_START_ADDR_TELE + idx) & 0xFF) };
...
static kal_uint8 gOV12D_CamInfo[CAMERA_MODULE_INFO_LENGTH];
...
static void read_eeprom_CamInfo(void)
{
	...
	gOV12D_CamInfo[0] = get_byte[0];
	gOV12D_CamInfo[1] = get_byte[1];
	gOV12D_CamInfo[2] = get_byte[6];
	gOV12D_CamInfo[3] = get_byte[7];
	gOV12D_CamInfo[4] = get_byte[8];
	gOV12D_CamInfo[5] = get_byte[9];
	gOV12D_CamInfo[6] = get_byte[10];
	gOV12D_CamInfo[7] = get_byte[11];
}
#endif
...
static kal_int32 write_Module_data(ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
	...
	if(pStereodata != NULL) {
		...
		if ((pStereodata->uSensorId == OV12D_SENSOR_ID)
				&& ((data_base == OV12D_STEREO_START_ADDR_WIDE)|| (data_base == OV12D_STEREO_START_ADDR_TELE))
				&& ((data_length == DUALCAM_CALI_DATA_LENGTH) || (data_length == DUALCAM_CALI_DATA_LENGTH_QCOM_MAIN))) {
			pr_debug("OV12D Write: %x %x %x %x %x %x %x %x\n", pData[0], pData[39], pData[40], pData[1556],
			...
			write_eeprom_protect(1);
			msleep(6);
			if (data_base == OV12D_STEREO_START_ADDR_TELE) {
				pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE));
				msleep(6);
				pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE+39));
				msleep(6);
				pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE+40));
				msleep(6);
				pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE+1556));
				msleep(6);
				pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE+1557));
				msleep(6);
				pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE+1558));
				msleep(6);
				pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE+1559));
				msleep(6);
				pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_TELE+1560));
				msleep(6);
			} else {
				pr_debug("com_0:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE));
				msleep(6);
				pr_debug("com_39:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE+39));
				msleep(6);
				pr_debug("innal_40:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE+40));
				msleep(6);
				pr_debug("innal_1556:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE+1556));
				msleep(6);
				pr_debug("tail1_1557:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE+1557));
				msleep(6);
				pr_debug("tail2_1558:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE+1558));
				msleep(6);
				pr_debug("tail3_1559:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE+1559));
				msleep(6);
				pr_debug("tail4_1560:0x%x\n", read_cmos_eeprom_8(OV12D_STEREO_START_ADDR_WIDE+1560));
				msleep(6);
			}
			pr_debug("OV12D write_Module_data Write end\n");
		}else if ((pStereodata->uSensorId == OV12D_SENSOR_ID)
			&& (data_base == OV12D_AESYNC_START_ADDR)
			...
			pr_debug("readback main aesync: %x %x %x %x %x %x %x %x\n"
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR)
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR+1)
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR+2)
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR+3)
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR+4)
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR+5)
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR+6)
				, read_cmos_eeprom_8(OV12D_AESYNC_START_ADDR+7));
		}else {
			pr_err("Invalid Sensor id:0x%x write_gm1 eeprom\n", pStereodata->uSensorId);
			return IMGSENSOR_RETURN_ERROR;
		}
	} else {
		pr_err("OV12Dwrite_Module_data pStereodata is null\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	return ret;
}
...
#if 0
static kal_uint16 OV12D_PDC_setting[8*2];
static kal_uint16 OV12D_PDC_setting_burst[720*2];
...
static void read_sensor_Cali(void)
{
	...
	for (idx = 0; idx <8; idx++) {
		...
		OV12D_PDC_setting[2*idx] = sensor_PDC_addr1;
		OV12D_PDC_setting[2*idx + 1] = read_cmos_eeprom_8(eeprom_PDC_addr);
	}
	for (idx = 8; idx < 728; idx++) {
		eeprom_PDC_addr = 0x24A4 + idx;
		sensor_PDC_addr2 = 0x5900 + idx - 8;
		//OV12D_PDC_setting[2*idx] = sensor_PDC_addr2;
		//OV12D_PDC_setting[2*idx + 1] = read_cmos_eeprom_8(eeprom_PDC_addr);
		OV12D_PDC_setting_burst[2*(idx-8)] = sensor_PDC_addr2;
		OV12D_PDC_setting_burst[2*(idx-8) + 1] = read_cmos_eeprom_8(eeprom_PDC_addr);
	}
}

static kal_uint16 OS12D40_burst_write_cmos_sensor(
					kal_uint16 *para, kal_uint32 len)
{
	char puSendCmd[I2C_BUFFER_LEN];
	kal_uint32 tosend, IDX;
...
{
	OS12D40_table_write_cmos_sensor(OV12D_PDC_setting,
		sizeof(OV12D_PDC_setting)/sizeof(kal_uint16));
	OS12D40_burst_write_cmos_sensor(OV12D_PDC_setting_burst,
		sizeof(OV12D_PDC_setting_burst)/sizeof(kal_uint16));
}
	...
	if (*sensor_id == imgsensor_info.sensor_id) {
		...
		printk("i2c write id: 0x%x, sensor id: 0x%x\n",
			...
			if(deviceInfo_register_value == 0x00){
					register_imgsensor_deviceinfo("Cam_r0", DEVICE_VERSION_OV12D, imgsensor_info.module_id);
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	*(feature_data + 1) = imgsensor_info.min_shutter;
		...
		case SENSOR_FEATURE_GET_MODULE_INFO:
			LOG_INF("OV12D GET_MODULE_CamInfo:%d %d\n", *feature_para_len, *feature_data_32);
			*(feature_data_32 + 1) = (gOV12D_CamInfo[1] << 24)
						| (gOV12D_CamInfo[0] << 16)
						| (gOV12D_CamInfo[3] << 8)
						| (gOV12D_CamInfo[2] & 0xFF);
			*(feature_data_32 + 2) = (gOV12D_CamInfo[5] << 24)
						| (gOV12D_CamInfo[4] << 16)
						| (gOV12D_CamInfo[7] << 8)
						| (gOV12D_CamInfo[6] & 0xFF);
			break;
		/*Henry.Chang@Camera.Driver add for 18531 ModuleSN*/
		case SENSOR_FEATURE_GET_MODULE_SN:
			LOG_INF("OV12D GET_MODULE_SN:%d %d\n", *feature_para_len, *feature_data_32);
			if (*feature_data_32 < CAMERA_MODULE_SN_LENGTH/4)
				*(feature_data_32 + 1) = (gOV12D_SN[4*(*feature_data_32) + 3] << 24)
							| (gOV12D_SN[4*(*feature_data_32) + 2] << 16)
							| (gOV12D_SN[4*(*feature_data_32) + 1] << 8)
							| (gOV12D_SN[4*(*feature_data_32)] & 0xFF);
			break;
		/*Henry.Chang@camera.driver 20181129, add for sensor Module SET*/
