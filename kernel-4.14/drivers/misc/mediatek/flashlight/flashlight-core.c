...
#if defined (CONFIG_PRIZE_FLASH_HW_TOUCH)

#define FLASHLIGHT_TORCH_DUTY       6
...
#if defined (CONFIG_PRIZE_FLASH_HW_TOUCH)
static int flashlight_hwstate = 0;
#endif
		...
		struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_ONOFF:
		printk("FLASH_IOC_SET_ONOFF(%d,%d,%d): %d\n",
				type, ct, part, fl_arg.arg);
//prize add by lipengpeng 20210315 start 
		if(type == 0 && ct == 0)
			flashlight_state = fl_arg.arg;
//prize add by lipengpeng 20210315 end 
		mutex_lock(&fl_mutex);
...
//prize add by lipengpeng 20210303 start 
static ssize_t flashlight_hwtorch_show(
		struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", flashlight_hwstate);
}

static ssize_t flashlight_hwtorch_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
  	struct flashlight_dev *fdev;
	...
	int type, ct, part,part_id;
	int ret;
	flashlight_hwstate = *buf - '0';
	pr_debug("flashlight_torch_store entry  flashlight_hwstate=%d\n",flashlight_hwstate);

	/* find flashlight device */
	mutex_lock(&fl_mutex);
	fdev = flashlight_find_dev_by_index(
			flashlight_get_type_index(1), // set type 1;it mean rear flash by zhuzhengjiang 20190816
	...
	printk("_flashlight_ioctl fl_dev_arg.arg=%d fl_dev_arg.channel=%d type=%d ct=%d part=%d\n",fl_dev_arg.arg,fl_dev_arg.channel,type,ct,part);
	if (flashlight_verify_index(type, ct, part)) {
		pr_err("Failed with error index\n");
		return -EINVAL;
	}
	...
	if (fdev->ops) {
		...
		//FLASH_IOC_SET_ONOFF
		fl_dev_arg.arg = flashlight_hwstate;
		if (fdev->ops->flashlight_ioctl(FLASH_IOC_SET_ONOFF,
				(unsigned long)&fl_dev_arg)) {
			pr_err("Failed to set on/off\n");
			return -EFAULT;
...
static DEVICE_ATTR(flashlight_hwtorch, 0644, flashlight_hwtorch_show, flashlight_hwtorch_store);
/* prize add by liaoxingen for hw torch end*/
//prize add by lipengpeng 20210303 start 
...
//prize add by lipengpeng 20210303 start 
	if (device_create_file(flashlight_device,
				&dev_attr_flashlight_hwtorch)) {
		pr_info("Failed to create device file(torch) \n");
		goto err_create_hwtorch_device_file;
	}
//prize add by lipengpeng 20210303 start 
#endif
	/* init flashlight */
...
err_create_torch_device_file:
 ...
 #if defined (CONFIG_PRIZE_FLASH_HW_TOUCH)
//prize add by lipengpeng 20210303 start 
 err_create_hwtorch_device_file:
     device_remove_file(flashlight_device, &dev_attr_flashlight_hwtorch);
//prize add by lipengpeng 20210303 start 
#endif
