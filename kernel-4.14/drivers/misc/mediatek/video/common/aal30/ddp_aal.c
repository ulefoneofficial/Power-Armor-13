	...
	enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	if (atomic_read(&g_aal_backlight_notified) == 0)
		backlight_value = 0;
	//prize-Solve slow sliding adjustment brightness, low brightness part adjustment, the screen slightly jitter-pengzhipeng-20210311-start
#ifdef CONFIG_MTK_AAL_SUPPORT
	//if (ret == 0)
	...
	if (ret == 0)
		ret |= disp_pwm_set_backlight_cmdq(DISP_PWM0,
			backlight_value, cmdq);
#endif
	//prize-Solve slow sliding adjustment brightness, low brightness part adjustment, the screen slightly jitter-pengzhipeng-20210311-end
