...
static void usbc_pincfg_lookup(struct usbtypc *typec)
{
	...
	//add by wolf for usb switch 20210409
	/*********************************************************/
	typec->pin_cfg->switch_usb_init = 
		pinctrl_lookup_state(typec->pinctrl, "redrv_switch_usb_init");
	if (IS_ERR(typec->pin_cfg->switch_usb_init)) {
		usbc_dbg(K_ERR, "Can *NOT* find redrv_switch_usb_init\n");
		typec->pin_cfg->switch_usb_init = NULL;
	} else
		usbc_dbg(K_DEBUG, "Find redrv_switch_usb_init\n");

	typec->pin_cfg->switch_usb_high = 
		pinctrl_lookup_state(typec->pinctrl, "redrv_switch_usb_high");
	if (IS_ERR(typec->pin_cfg->switch_usb_high)) {
		usbc_dbg(K_ERR, "Can *NOT* find redrv_switch_usb_high\n");
		typec->pin_cfg->switch_usb_high = NULL;
	} else
		usbc_dbg(K_DEBUG, "Find redrv_switch_usb_high\n");

	typec->pin_cfg->switch_usb_low = 
		pinctrl_lookup_state(typec->pinctrl, "redrv_switch_usb_low");
	if (IS_ERR(typec->pin_cfg->switch_usb_low)) {
		usbc_dbg(K_ERR, "Can *NOT* find redrv_switch_usb_low\n");
		typec->pin_cfg->switch_usb_low = NULL;
	} else
		usbc_dbg(K_DEBUG, "Find redrv_switch_usb_low\n");	
...
static int usb_switch_init(struct usbtypc *typec)
{
	int retval = 0;
	if(typec->pin_cfg->switch_usb_init)
	{
		usbc_dbg(K_ERR,"wolf:pinctrl_select_state switch_usb_init\n");
		pinctrl_select_state(typec->pinctrl,typec->pin_cfg->switch_usb_init);
	}
 
	return retval;
}
static int usbswitch_event_handler(void *unused)
{
	struct usbtypc *typec;
	int gpio_val = 0;
	...
	do
	{
		gpio_val = gpio_get_value(typec->usb_sw.usb_switch_irq_gpio);
		// usbc_dbg(K_ERR, "%s:wolf:gpio_val = %d\n",__func__,gpio_val);
		if(gpio_val == 0)
		{
			usbc_dbg(K_ERR, "%s:wolf:switch_usb_high\n",__func__);
			pinctrl_select_state(typec->pinctrl,typec->pin_cfg->switch_usb_high);
		}else
		{
			usbc_dbg(K_ERR, "%s:wolf:switch_usb_low\n",__func__);
			pinctrl_select_state(typec->pinctrl,typec->pin_cfg->switch_usb_low);
		}
		msleep(50);
...
static int usbc_pinctrl_probe(struct platform_device *pdev)
{
	...
	//add by wolf for usb camera switch 20210409 start
	np_usb_irq = pdev->dev.of_node;
	if(np_usb_irq)
	{
		typec->usb_sw.usb_switch_irq_gpio = of_get_named_gpio(np_usb_irq,"usb_sw_pin",0);
		usbc_dbg(K_ERR, "wolf:typec->usb_sw.usb_switch_irq_gpio = %d\n",typec->usb_sw.usb_switch_irq_gpio);
		if(typec->usb_sw.usb_switch_irq_gpio < 0)
		{
			usbc_dbg(K_ERR, "wolf:usb_switch_irq_gpio error\n");
		}
		else
		{
			ret = gpio_request(typec->usb_sw.usb_switch_irq_gpio,"usb_sw_pin");
			if(ret < 0)
			{
				usbc_dbg(K_ERR, "wolf:usb_switch_irq_gpio gpio_request error\n");
			}
			else
			{
				usbc_dbg(K_ERR, "wolf:usb_switch_irq_gpio gpio_request error\n");
	...
	usb_switch_init(typec);
	// usb_redriver_init(typec);
	// usb3_switch_init(typec);

	//add by wolf for S7 usb switch 20210409
	thread = kthread_run(usbswitch_event_handler, 0, "usb_switch");
	if (IS_ERR(thread)) {
		err = PTR_ERR(thread);
		usbc_dbg(K_ERR,"failed to create kernel thread: %d", err);
	}
...
{
	int ret = 0;
	usbc_dbg(K_ERR, "register usbc pinctrl start!!\n");
