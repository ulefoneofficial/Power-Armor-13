...
//prize add by huarui 20201010 start
#if defined(CONFIG_HISENSE_HSDBG)
#include <hisense/hsdbg.h>
#endif
//prize add by huarui 20201010 end
...
int fts_reset_test(void)
{
    ...
    gpio_direction_output(fts_data->pdata->reset_gpio, 0);
	msleep(1);
    ret = fts_read_reg(FTS_REG_VENDOR_ID, &vid);
    gpio_direction_output(fts_data->pdata->reset_gpio, 1);
    if (ret < 0){
		return 1;
	}else{
...
/*prize-add by anhengxuan 20201121 for spi_cs suspend function,begin*/	
    ts->spi_cs_pins_suspend = pinctrl_lookup_state(ts->pinctrl, "spi_cs_suspend");
    if (IS_ERR_OR_NULL(ts->spi_cs_pins_suspend)) {
        FTS_ERROR("spi_cs_suspend not found");
        ret = PTR_ERR(ts->spi_cs_pins_suspend);
    }

	ts->spi_cs_pins_resume = pinctrl_lookup_state(ts->pinctrl, "spi_cs_resume");
    if (IS_ERR_OR_NULL(ts->spi_cs_pins_resume)) {
        FTS_ERROR("spi_cs_suspend not found");
        ret = PTR_ERR(ts->spi_cs_pins_resume);
    }
/*prize-add by anhengxuan 20201121 for spi_cs suspend function,end*/
...
err_pinctrl_get:
    ...
    ts->pins_active = NULL;
	/*prize-add by anhengxuan 20201121 for spi_cs suspend function,begin*/
	ts->spi_cs_pins_suspend=NULL;
	ts->spi_cs_pins_resume=NULL;
	/*prize-add by anhengxuan 20201121 for spi_cs suspend function,end*/
    return ret;
}
...
/*prize-add by anhengxuan 20201121 for spi_cs suspend function,begin*/
	ret = pinctrl_select_state(ts->pinctrl, ts->spi_cs_pins_resume);
        if (ret < 0) {
...
/*prize-add by anhengxuan 20201121 for spi_cs suspend function,begin*/
	ret = pinctrl_select_state(ts->pinctrl, ts->spi_cs_pins_suspend);
        if (ret < 0) {
...
//prize add by lipengpeng 20201009 start
static int vsensor_power_init(struct fts_ts_data *ts_data)
{
    int ret = 0;
    printk("lpp---vsensor_power_init---\n");
    ts_data->vddsensor = regulator_get(ts_data->dev, "VFP");
    if (IS_ERR_OR_NULL(ts_data->vddsensor)) {
        ret = PTR_ERR(ts_data->vddsensor);
        printk("get vddsensor regulator failed,ret=%d", ret);
        return ret;
    }
        ret = regulator_set_voltage(ts_data->vddsensor, 3000000, 3000000);
        if (ret) {
            printk("vddsensor regulator set_vtg failed ret=%d", ret);
            return ret;
        }

    ret = regulator_enable(ts_data->vddsensor);
    if (ret) {
        FTS_ERROR("fail to enable power(regulator)");
    }
    return ret;
}
//prize add by lipengpeng 20201009 end 
...
//prize add by huarui 20201010 start
#if defined(CONFIG_HISENSE_HSDBG)
	hsdbg_set_ctp_str(&hsdbg_ctp_data.chiptest,"focaltech");
	memset(info_buf, 0, sizeof(info_buf));
	//sprintf(info_buf, "0x%02x%02x", ts_data->ic_info.ids.chip_idh,
	//			ts_data->ic_info.ids.chip_idl);
	//hsdbg_set_ctp_str(&hsdbg_ctp_data.chiptest,"focaltech");
	fts_read_reg(FTS_REG_VENDOR_ID, &info_buf[10]);
	sprintf(info_buf, "0x%02X", info_buf[10]);
	printk("%s: %02x %s\n",__func__,info_buf[10],info_buf);
	hsdbg_set_ctp_str(&hsdbg_ctp_data.fwmoduleid,info_buf);
#endif
//prize add by huarui 20201010 end
...
//prize add by lipengpeng 20201009 start
    ret = vsensor_power_init(ts_data);
    if (ret) {
        printk("fail vsensor_power_init regulator)");
    }
//prize add by lipengpeng 20201009 end 
...
/*bug:HLTE322E--anhengxuan-20201102-fix TP current leakage,begin*/
       // if (!ts_data->ic_info.is_incell) {
/*bug:HLTE322E--anhengxuan-20201102-fix TP current leakage,end*/
#if FTS_POWER_SOURCE_CUST_EN
