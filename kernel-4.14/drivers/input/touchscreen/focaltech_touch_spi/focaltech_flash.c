...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    struct fts_upgrade *upg = fwupgrade;
//prize add by huarui 20201010 start
#if defined(CONFIG_HISENSE_HSDBG)||defined(CONFIG_HISENSE_PRODUCT_DEVINFO)
	char info_buf[50] = {0};
#endif
#if defined(CONFIG_HISENSE_PRODUCT_DEVINFO)
	char vid_buf[20] = {0};
...
//prize add by huarui 20201010 start
	...
	fts_read_reg(FTS_REG_FW_VER, &info_buf[10]);
	sprintf(info_buf, "0x%02X", info_buf[10]);
	hsdbg_set_ctp_str(&hsdbg_ctp_data.fwversion,info_buf);
#endif

#if defined(CONFIG_HISENSE_PRODUCT_DEVINFO)
	memset(info_buf, 0, sizeof(info_buf));
	fts_read_reg(FTS_REG_FW_VER, &vid_buf[0]);
	fts_read_reg(FTS_REG_VENDOR_ID, &vid_buf[10]);
	sprintf(info_buf, "Fw version:0x%02x Module id:0x%02x", vid_buf[0],vid_buf[10]);
	productinfo_register(PRODUCTINFO_CTP_ID,info_buf,NULL);
#endif
//prize add by huarui 20201010 end
}
