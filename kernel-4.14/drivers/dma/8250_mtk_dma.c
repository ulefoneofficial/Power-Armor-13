...
static void mtk_dma_tx_flush(struct dma_chan *chan)
{
    ...
    pr_err("%s: entry loopcnt = %d\n", __func__, atomic_read(&c->loopcnt));
    dump_stack();
	
	if (mtk_dma_chan_read(c, VFF_FLUSH) == 0) {
		pr_err("%s: entry2 loopcnt = %d\n", __func__, atomic_read(&c->loopcnt));
			...
			struct dma_slave_config *cfg)
{
    ...
    pr_err("%s: entry cfg->direction = %d\n", __func__, cfg->direction);
    dump_stack();
