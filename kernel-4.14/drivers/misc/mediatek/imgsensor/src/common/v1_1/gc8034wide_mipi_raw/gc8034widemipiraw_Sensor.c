...
#define PFX "gc8034wide_camera_sensor"
...
#include "gc8034widemipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
//#define PFX "gc8034wide_camera_sensor"
#define LOG_1 LOG_INF("GC8034WIDE,MIPI 4LANE\n")
extern int curr_sensor_id;
/****************************   Modify end    *******************************************/
/*#define GC8034WIDE_DEBUG*/
#if 1 //#ifdef GC8034WIDE_DEBUG
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)
#else
#define LOG_INF(format, args...)
#endif
#if 0
static struct agc_params_struct GC8034WIDE_AGC_Param[AG_INDEX] = {
	{
		64, /* 1.00x */
		{
			{ 0xfe, 0x00 },
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.sensor_id = GC8034WIDE_SENSOR_ID,
...
static struct gc8034wide_otp gc8034wide_otp_info = {0};
#if 1 //#ifdef GC8034WIDESUBSUBOTP_FOR_CUSTOMER
static kal_uint8 LSC_ADDR[4] = { 0x0e, 0x20, 0x1a, 0x88 };
#endif
static kal_uint8 gc8034wide_read_otp(kal_uint8 page, kal_uint8 addr)
{
	LOG_INF("E!\n");
...
static void gc8034wide_read_otp_group(kal_uint8 page, kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint8 i;
...
static void gc8034wide_gcore_read_otp_info(void)
{
	kal_uint8  flagdd = 0;
	kal_uint16 i = 0, j = 0;
	kal_uint8  temp = 0;
	...
	kal_uint8  af[4] = { 0 };
#endif
#if 0 //#ifdef GC8034WIDESUBSUBOTP_DEBUG
	kal_uint8  debug[128] = { 0 };
#endif
	memset(&gc8034wide_otp_info, 0, sizeof(gc8034wide_otp_info));

	/* Static Defective Pixel */
	flagdd = gc8034wide_read_otp(0, 0x0b);
	LOG_INF("GC8034WIDE_OTP_DD : flag_dd = 0x%x\n", flagdd);

	switch (flagdd & 0x03) {
	case 0x00:
		LOG_INF("GC8034WIDE_OTP_DD is Empty !!\n");
		gc8034wide_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("GC8034WIDE_OTP_DD is Valid!!\n");
		gc8034wide_otp_info.dd_cnt = gc8034wide_read_otp(0, 0x0c) + gc8034wide_read_otp(0, 0x0d);
		gc8034wide_otp_info.dd_flag = 0x01;
		LOG_INF("GC8034WIDE_OTP : total_number = %d\n", gc8034wide_otp_info.dd_cnt);
		break;
	case 0x02:
	case 0x03:
		LOG_INF("GC8034WIDE_OTP_DD is Invalid !!\n");
		gc8034wide_otp_info.dd_flag = 0x02;
		break;
	default:
		break;
	}

#if 1 //#ifdef GC8034WIDEOTP_FOR_CUSTOMER
	flag_module = gc8034wide_read_otp(9, 0x6f);
	flag_wb = gc8034wide_read_otp(9, 0x5e);
	flag_af = gc8034wide_read_otp(3, 0x3a);
	LOG_INF("GC8034WIDE_OTP : flag_module = 0x%x , flag_wb = 0x%x , flag_af = 0x%x\n", flag_module, flag_wb, flag_af);

	/* INFO & WB & AF */
	for (index = 0; index < 2; index++) {
		switch ((flag_module << (2 * index)) & 0x0c) {
		case 0x00:
			LOG_INF("GC8034WIDE_OTP_INFO group %d is Empty !!\n", index + 1);
			break;
		case 0x04:
			LOG_INF("GC8034WIDE_OTP_INFO group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034wide_read_otp_group(9, INFO_ROM_START + index * INFO_WIDTH, &info[0], INFO_WIDTH);
			for (i = 0; i < INFO_WIDTH - 1; i++)
				check += info[i];

			if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
				gc8034wide_otp_info.module_id = info[0];
				gc8034wide_otp_info.lens_id = info[1];
				gc8034wide_otp_info.vcm_driver_id = info[2];
				gc8034wide_otp_info.vcm_id = info[3];
				gc8034wide_otp_info.year = info[4];
				gc8034wide_otp_info.month = info[5];
				gc8034wide_otp_info.day = info[6];
			} else
				LOG_INF("GC8034WIDE_OTP_INFO Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034WIDE_OTP_INFO group %d is Invalid !!\n", index + 1);
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC8034WIDE_OTP_WB group %d is Empty !!\n", index + 1);
			gc8034wide_otp_info.wb_flag = gc8034wide_otp_info.wb_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("GC8034WIDE_OTP_WB group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034wide_read_otp_group(9, WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
			for (i = 0; i < WB_WIDTH - 1; i++)
				check += wb[i];

			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				gc8034wide_otp_info.rg_gain = (wb[0] | ((wb[1] & 0xf0) << 4)) > 0 ?
					(wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
				gc8034wide_otp_info.bg_gain = (((wb[1] & 0x0f) << 8) | wb[2]) > 0 ?
					(((wb[1] & 0x0f) << 8) | wb[2]) : 0x400;
				gc8034wide_otp_info.wb_flag = gc8034wide_otp_info.wb_flag | 0x01;
			} else
				LOG_INF("GC8034WIDE_OTP_WB Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034WIDE_OTP_WB group %d is Invalid !!\n", index + 1);
			gc8034wide_otp_info.wb_flag = gc8034wide_otp_info.wb_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC8034WIDE_OTP_GOLDEN group %d is Empty !!\n", index + 1);
			gc8034wide_otp_info.golden_flag = gc8034wide_otp_info.golden_flag | 0x00;
			break;
		case 0x40:
			LOG_INF("GC8034WIDE_OTP_GOLDEN group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034wide_read_otp_group(9, GOLDEN_ROM_START + index * GOLDEN_WIDTH, &golden[0], GOLDEN_WIDTH);
			for (i = 0; i < GOLDEN_WIDTH - 1; i++)
				check += golden[i];

			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				gc8034wide_otp_info.golden_rg = (golden[0] | ((golden[1] & 0xf0) << 4)) > 0 ?
					(golden[0] | ((golden[1] & 0xf0) << 4)) : RG_TYPICAL;
				gc8034wide_otp_info.golden_bg = (((golden[1] & 0x0f) << 8) | golden[2]) > 0 ?
					(((golden[1] & 0x0f) << 8) | golden[2]) : BG_TYPICAL;
				gc8034wide_otp_info.golden_flag = gc8034wide_otp_info.golden_flag | 0x01;
			} else
				LOG_INF("GC8034WIDE_OTP_GOLDEN Check sum %d Error !!\n", index + 1);

			break;
		case 0x80:
		case 0xc0:
			LOG_INF("GC8034WIDE_OTP_GOLDEN group %d is Invalid !!\n", index + 1);
			gc8034wide_otp_info.golden_flag = gc8034wide_otp_info.golden_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			LOG_INF("GC8034WIDE_OTP_AF group %d is Empty !!\n", index + 1);
			gc8034wide_otp_info.af_flag = gc8034wide_otp_info.af_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("GC8034WIDE_OTP_AF group %d is Valid !!\n", index + 1);
			check = 0;
			gc8034wide_read_otp_group(3, AF_ROM_START + index * AF_WIDTH, &af[0], AF_WIDTH);
			for (i = 0; i < AF_WIDTH - 1; i++)
				check += af[i];

			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				gc8034wide_otp_info.af_infinity = ((af[0] << 4) & 0x0f00) + af[1];
				gc8034wide_otp_info.af_macro = ((af[0] << 8) & 0x0f00) + af[2];
				gc8034wide_otp_info.af_flag = gc8034wide_otp_info.af_flag | 0x01;
			} else
				LOG_INF("GC8034WIDE_OTP_AF Check sum %d Error !!\n", index + 1);
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034WIDE_OTP_AF group %d is Invalid !!\n", index + 1);
			gc8034wide_otp_info.af_flag = gc8034wide_otp_info.af_flag | 0x02;
			break;
		default:
			break;
		}
	...
	/* LSC */
	flag_lsc = gc8034wide_read_otp(3, 0x43);
	gc8034wide_otp_info.lsc_flag = 2;
	for (index = 0; index < 2; index++) {
		switch ((flag_lsc << (2 * index)) & 0x0c) {
		case 0x00:
			LOG_INF("GC8034WIDE_OTP_LSC group %d is Empty !\n", index + 1);
			break;
		case 0x04:
			LOG_INF("GC8034WIDE_OTP_LSC group %d is Valid !\n", index + 1);
			gc8034wide_otp_info.lsc_flag = index;
			break;
		case 0x08:
		case 0x0c:
			LOG_INF("GC8034WIDE_OTP_LSC group %d is Invalid !!\n", index + 1);
			break;
		default:
			break;
		}
	...
	LOG_INF("GC8034WIDE_OTP_INFO:module_id=0x%x\n", gc8034wide_otp_info.module_id);
	LOG_INF("GC8034WIDE_OTP_INFO:lens_id=0x%x\n", gc8034wide_otp_info.lens_id);
	LOG_INF("GC8034WIDE_OTP_INFO:vcm_id=0x%x\n", gc8034wide_otp_info.vcm_id);
	LOG_INF("GC8034WIDE_OTP_INFO:vcm_driver_id=0x%x\n", gc8034wide_otp_info.vcm_driver_id);
	LOG_INF("GC8034WIDE_OTP_INFO:data=%d-%d-%d\n", gc8034wide_otp_info.year, gc8034wide_otp_info.month, gc8034wide_otp_info.day);
	LOG_INF("GC8034WIDE_OTP_WB:r/g=0x%x\n", gc8034wide_otp_info.rg_gain);
	LOG_INF("GC8034WIDE_OTP_WB:b/g=0x%x\n", gc8034wide_otp_info.bg_gain);
	LOG_INF("GC8034WIDE_OTP_GOLDEN:golden_rg=0x%x\n", gc8034wide_otp_info.golden_rg);
	LOG_INF("GC8034WIDE_OTP_GOLDEN:golden_bg=0x%x\n", gc8034wide_otp_info.golden_bg);
	LOG_INF("GC8034WIDE_OTP_AF:infitiy=0x%x\n", gc8034wide_otp_info.af_infinity);
	LOG_INF("GC8034WIDE_OTP_AF:macro=0x%x\n", gc8034wide_otp_info.af_macro);
#endif

	/* chip regs */
	gc8034wide_otp_info.reg_flag = gc8034wide_read_otp(2, 0x4e);

	if (gc8034wide_otp_info.reg_flag == 1)
		for (i = 0; i < 5; i++) {
			temp = gc8034wide_read_otp(2, 0x4f + 5 * i);
			for (j = 0; j < 2; j++)
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {
					gc8034wide_otp_info.reg_page[gc8034wide_otp_info.reg_num] = (temp >> (4 * j)) & 0x03;
					gc8034wide_otp_info.reg_addr[gc8034wide_otp_info.reg_num] =
						gc8034wide_read_otp(2, 0x50 + 5 * i + 2 * j);
					gc8034wide_otp_info.reg_value[gc8034wide_otp_info.reg_num] =
						gc8034wide_read_otp(2, 0x50 + 5 * i + 2 * j + 1);
					gc8034wide_otp_info.reg_num++;
				}
		}

#if 0 //#ifdef GC8034WIDEOTP_DEBUG
	for(i = 0; i < 9; i++) {
		gc8034wide_read_otp_group(i, 0, &debug[0], 128);
		for(j = 0; j < 128; j++) {
			LOG_INF("GC8034WIDE_OTP_DEBUG: Page%d: addr = 0x%x, value = 0x%x;\n", i, j, debug[j]);
		}
	}
#endif
}

static void gc8034wide_gcore_check_prsel(void)
{
	kal_uint8 product_level = 0;

	product_level = gc8034wide_read_otp(2, 0x68) & 0x07;
...
static void gc8034wide_gcore_update_dd(void)
{
	kal_uint8 state = 0;

	if (0x01 == gc8034wide_otp_info.dd_flag) {
		LOG_INF("GC8034WIDE_OTP_AUTO_DD start !\n");
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0x79, 0x2e);
		write_cmos_sensor(0x7b, gc8034wide_otp_info.dd_cnt);
...
#if 1 //#ifdef GC8034WIDEOTP_FOR_CUSTOMER
static void gc8034wide_gcore_update_wb(void)
{
	...
	if (0x01 == (gc8034wide_otp_info.golden_flag & 0x01)) {
		rg_typical = gc8034wide_otp_info.golden_rg;
		bg_typical = gc8034wide_otp_info.golden_bg;
		LOG_INF("GC8034WIDE_OTP_UPDATE_AWB:rg_typical = 0x%x, bg_typical = 0x%x\n", rg_typical, bg_typical);
	} else {
	...
	if (0x01 == (gc8034wide_otp_info.wb_flag & 0x01)) {
		r_gain_current = 2048 * rg_typical / gc8034wide_otp_info.rg_gain;
		b_gain_current = 2048 * bg_typical / gc8034wide_otp_info.bg_gain;
		g_gain_current = 2048;
		...
		LOG_INF("GC8034WIDE_OTP_UPDATE_AWB:r = 0x%x, g = 0x%x, b = 0x%x\n", r_gain, g_gain, b_gain);
...
static void gc8034wide_gcore_update_lsc(void)
{
	kal_uint8 state = 0;

	if (0x02 != gc8034wide_otp_info.lsc_flag) {
		write_cmos_sensor(0xfe, 0x00);
		write_cmos_sensor(0x78, 0x9b);
		write_cmos_sensor(0x79, 0x0d);
		write_cmos_sensor(0x7a, LSC_NUM);
		write_cmos_sensor(0x7c, LSC_ADDR[gc8034wide_otp_info.lsc_flag * 2]);
		write_cmos_sensor(0x7d, LSC_ADDR[gc8034wide_otp_info.lsc_flag * 2 + 1]);
		write_cmos_sensor(0x6e, 0x01);
		write_cmos_sensor(0xfe, 0x01);
...
static void gc8034wide_gcore_update_chipversion(void)
{
	kal_uint8 i = 0;

	LOG_INF("GC8034WIDE_OTP_UPDATE_CHIPVERSION:reg_num = %d\n", gc8034wide_otp_info.reg_num);

	if (gc8034wide_otp_info.reg_flag)
		for (i = 0; i < gc8034wide_otp_info.reg_num; i++) {
			write_cmos_sensor(0xfe, gc8034wide_otp_info.reg_page[i]);
			write_cmos_sensor(gc8034wide_otp_info.reg_addr[i], gc8034wide_otp_info.reg_value[i]);
			LOG_INF("GC8034WIDE_OTP_UPDATE_CHIP_VERSION: P%d:0x%x -> 0x%x\n",
			gc8034wide_otp_info.reg_page[i], gc8034wide_otp_info.reg_addr[i], gc8034wide_otp_info.reg_value[i]);
		}
}

static void gc8034wide_gcore_update_otp(void)
{
	gc8034wide_gcore_update_dd();
	gc8034wide_gcore_check_prsel();
#if 1 //#ifdef GC8034WIDEOTP_FOR_CUSTOMER
	gc8034wide_gcore_update_wb();
	gc8034wide_gcore_update_lsc();
#endif
	gc8034wide_gcore_update_chipversion();
}

static void gc8034wide_gcore_enable_otp(kal_uint8 state)
{
	...
	if (state) {
		...
		LOG_INF("GC8034WIDE_OTP: Enable OTP!\n");
	} else {
		otp_en = otp_en & 0xf7;
		otp_clk = otp_clk & 0xfe;
		write_cmos_sensor(0xf4, otp_en);
		write_cmos_sensor(0xf2, otp_clk);
		LOG_INF("GC8034WIDE_OTP: Disable OTP!\n");
	}
}

static void gc8034wide_gcore_identify_otp(void)
{
	gc8034wide_gcore_enable_otp(otp_open);
	gc8034wide_gcore_read_otp_info();
	gc8034wide_gcore_update_otp();
	gc8034wide_gcore_enable_otp(otp_close);
}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, 0xc0); //GC8034WIDE_MIRROR
...
static void fullsize_setting(void)
{
	...
	write_cmos_sensor(0x94, 0x09);//FullStartX
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	 ...
	 */
	if(curr_sensor_id  != 3) {
		printk("curr sensor idx is not match physical idx,so return err \n ");
		...
		do {
			*sensor_id = return_sensor_id()+2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("[gc8034wide_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
...
/*const kal_uint8 i2c_addr[] = {IMGSENSOR_WRITE_ID_1, IMGSENSOR_WRITE_ID_2};*/
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id()+2;
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("[gc8034wide_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("[gc8034wide_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 GC8034WIDE_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	GC8034WIDE_MIPI_RAW_SensorInit	*/
