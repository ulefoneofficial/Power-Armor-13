...
int set_shutdown_cond(int shutdown_cond)
{
	...
	printk("lpp----sdc.shutdown_status.is_soc_zero_percent=%d,shutdown_cond_flag=%d",sdc.shutdown_status.is_soc_zero_percent,shutdown_cond_flag);
...
static int shutdown_event_handler(struct shutdown_controller *sdd)
{
	...
	int vbat = battery_get_bat_voltage();
	int tmp = 25;
//prize add by lipengpeng 20210128 start 
	int now_current_charge;
	int now_is_charging=0;
//prize add by lipengpeng 20210128 end  
	now.tv_sec = 0;
	now.tv_nsec = 0;
	duraction.tv_sec = 0;
	duraction.tv_nsec = 0;
...
//prize add by lipengpeng 20210128 start 
	now_current_charge = battery_get_bat_current();

	if (now_current_charge >= 0)
		now_is_charging = 1;
//prize add by lipengpeng 20210128 start 
	...
	if (sdd->shutdown_status.is_soc_zero_percent) {
		if (current_ui_soc == 0) {
			...
			polling++;
//prize add by lipengpeng 20210128 start 
			if (duraction.tv_sec >= SHUTDOWN_TIME && now_is_charging !=1 ) {
//prize add by lipengpeng 20210128 start 
