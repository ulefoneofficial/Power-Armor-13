...
struct hynitron_fw_array hynitron_fw_grp[] = {
	...
	{ "capacitive_hynitron_cst3xx_update",   cst3xx_fw,   0x2111,0x11, CST148, (sizeof(cst3xx_fw))},
	{ "capacitive_hynitron_cst6xx_update",   cst6xx_fw,   0x2457,0x11, CST6928S, (sizeof(cst6xx_fw))},
...
static int hyn_find_fw_idx(u8 check_project_id)
{
	...
	if (0 != hyn_ts_data->config_project_id ) {
		for (i=0; i<ARRAY_SIZE(hynitron_fw_grp); i++) {
			if(check_project_id==1){
				if((hyn_ts_data->config_project_id  ==hynitron_fw_grp[i].project_id)
...
int hyn_boot_update_fw(struct i2c_client * client)
{
	...
	if(hyn_ts_data->apk_upgrade_flag==0){	
		...
		if(proj_id<0){
			HYN_ERROR("not find matched TP firmware,please check HYN_CHIP_PROJECT_ID_CONFIG(common.h) !\n"); 
			check_proj_id=0;
			proj_id=hyn_find_fw_idx(check_proj_id);
