...
int hyn_update_firmware_init(struct i2c_client *client)
{
	...
	if(ret<0){
		...
		//hyn_ts_data->chip_ic_fw_version=0x00;
	}
...
void hyn_ts_data_init(struct i2c_client *client)
{
	...
	hyn_ts_data->config_project_id		=HYN_CHIP_PROJECT_ID_CONFIG;
	hyn_ts_data->config_module_id		=HYN_CHIP_MODULE_ID_CONFIG;
	...
	hyn_ts_data->pdata->x_resolution     =HYN_X_DISPLAY_DEFAULT;
	hyn_ts_data->pdata->y_resolution     =HYN_Y_DISPLAY_DEFAULT;
	
	HYN_DEBUG("config data config_chip_type:0x%x,config_project_id:0x%x,config_module_id:0x%x,chip_ic_main_addr:0x%x. \r\n",
	hyn_ts_data->config_chip_type,hyn_ts_data->config_project_id,
	hyn_ts_data->config_module_id,hyn_ts_data->chip_ic_main_addr);
