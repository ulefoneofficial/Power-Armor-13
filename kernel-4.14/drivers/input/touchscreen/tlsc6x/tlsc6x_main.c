...
static struct hwinfo *tp_hwinfo = NULL;
...
static void tlsc6x_virtual_keys_init(void)
{
	...
	properties_kobj = kobject_create_and_add("board_properties", &g_tp_drvdata->input_dev->dev.kobj);
	if (properties_kobj) {
...
static int tlsc6x_hw_init(struct tlsc6x_data *drvdata)
{
	...
	#if 1
	reg_vdd = regulator_get(&client->dev, "vtouch");
	if (IS_ERR(reg_vdd)) {
	...
	/*set 2.8v*/
	ret = regulator_set_voltage(reg_vdd, 2800000, 2800000);
	if (ret)
		printk("regulator_set_voltage(%d) failed!\n", ret);
	
	ret = regulator_enable(reg_vdd);
	if (ret)
		printk("regulator_enable(%d) failed!\n", ret);
	...
	if (IS_ERR(sprd_tpd_class)) {
		...
		if (IS_ERR(sprd_ps_cmd_dev)) {
			...
			if (device_create_file(sprd_ps_cmd_dev, &dev_attr_proximity) < 0) {
				dev_err(&this_client->dev," tlsc6x error::create device file fail(%s)!\n", dev_attr_proximity.attr.name);
			}
		}
	}
...
ssize_t GTprocTpInfoRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u8 nLength;
...
ssize_t GTprocTpInfoWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)	
{
	return nCount;
}

static const struct file_operations GTProcTpInfo = { 
	.read = GTprocTpInfoRead,
...
//prize added by huarui, fts support, 20190111-start
	...
	sprintf(current_tp_info.chip,"Chsc6440_FW:0x%02x", 0x20);
    sprintf(current_tp_info.id,"0x%04x",0x6440);
    strcpy(current_tp_info.vendor,"chsc6440");
