...
#include "s5k3p3sxgqmipiraw_Sensor.h"

#define PFX "S5K3P3SXGQ_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3P3SXGQ_SENSOR_ID,
...
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
	//return; //for test
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor framerate = %d, min framelength should enable %d \n", framerate,min_framelength_en);
...
static void write_shutter(kal_uint16 shutter)
{
	...
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if (gain < BASEGAIN || gain > 32 * BASEGAIN) {
        printk(" liaojie s5k3p3sxgqmipiraw_Sensor Error gain setting");
    ...
    spin_unlock(&imgsensor_drv_lock);
    printk(" liaojie s5k3p3sxgqmipiraw_Sensor gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
	...
	switch(image_mirror)
		{
		   ...
		   case IMAGE_HV_MIRROR:
			     ...
			     break;
        default:
        printk(" liaojie s5k3p3sxgqmipiraw_Sensor Error image_mirror setting\n");
		}
}
...
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor func = %s \n",__func__);
...
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor E! \n");
    // full size 29.76fps
    // capture setting
// Pll Setting - VCO = 280Mhz
 write_cmos_sensor(0x0100, 0x0000);
...
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor E! currefps:%d\n",currefps);
    // full size 30fps
    // capture setting
// Pll Setting - VCO = 280Mhz
  write_cmos_sensor(0x0100, 0x0000);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			printk(" liaojie s5k3p3sxgqmipiraw_Sensor read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",read_cmos_sensor_8(0x0000),read_cmos_sensor_8(0x0001),read_cmos_sensor(0x0000));
			if (*sensor_id ==imgsensor_info.sensor_id) {
				printk(" liaojie s5k3p3sxgqmipiraw_Sensor i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			printk(" liaojie s5k3p3sxgqmipiraw_Sensor Read sensor id fail, id: 0x%x\n",*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor PLATFORM:MT6797,MIPI 4LANfunc = %s \n",__func__);
...
static kal_uint32 get_info(enum MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor scenario_id = %d\n", scenario_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		default:
			printk(" liaojie s5k3p3sxgqmipiraw_Sensor Error ScenarioId setting");
			preview(image_window, sensor_config_data);
...
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor framerate = %d\n ", framerate);
...
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
        	  if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
                ...
                spin_lock(&imgsensor_drv_lock);
                    ...
                    printk(" liaojie s5k3p3sxgqmipiraw_Sensor Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",framerate,imgsensor_info.cap.max_framerate/10);
		...
		default:  //coding with  preview scenario by default
			...
			spin_unlock(&imgsensor_drv_lock);
			//set_dummy();
			printk(" liaojie s5k3p3sxgqmipiraw_Sensor error scenario_id = %d, we use preview scenario \n", scenario_id);
			break;
	}
	return ERROR_NONE;
}
...
{
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor enable: %d\n", enable);
...
static kal_uint32 feature_control( MSDK_SENSOR_FEATURE_ENUM feature_id,UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	printk(" liaojie s5k3p3sxgqmipiraw_Sensor feature_id = %d ,name: %s \n", feature_id,prize_camera_feature[feature_id-3000].cmd_name);
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
            printk(" liaojie s5k3p3sxgqmipiraw_Sensor feature_Control imgsensor.pclk = %d,imgsensor.current_fps = %d\n", imgsensor.pclk,imgsensor.current_fps);
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			//printk(" liaojie s5k3p3sxgqmipiraw_Sensor SENSOR_FEATURE_GET_PDAF_DATA\n");
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
            printk(" liaojie s5k3p3sxgqmipiraw_Sensor current fps :%d\n", (UINT32)*feature_data);
			spin_lock(&imgsensor_drv_lock);
            imgsensor.current_fps = *feature_data;
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_SET_HDR:
            printk(" liaojie s5k3p3sxgqmipiraw_Sensor ihdr enable :%d\n", (BOOL)*feature_data);
			spin_lock(&imgsensor_drv_lock);
			imgsensor.ihdr_mode = *feature_data;
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_GET_CROP_INFO:
            printk(" liaojie s5k3p3sxgqmipiraw_Sensor SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
		...
		//	printk(" liaojie s5k3p3sxgqmipiraw_Sensor SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%lld\n", *feature_data);
		...
		case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
			printk(" liaojie s5k3p3sxgqmipiraw_Sensor SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY scenarioId:%lld\n", *feature_data);
...
//prize add by lipengpeng 20200402 end 
		...
		case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            printk(" liaojie s5k3p3sxgqmipiraw_Sensor SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",(UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
            //ihdr_write_shutter_gain((UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
			break;
        case SENSOR_FEATURE_SET_AWB_GAIN:
            break;
        case SENSOR_FEATURE_SET_HDR_SHUTTER:
            printk(" liaojie s5k3p3sxgqmipiraw_Sensor SENSOR_FEATURE_SET_HDR_SHUTTER LE=%d, SE=%d\n",(UINT16)*feature_data,(UINT16)*(feature_data+1));
