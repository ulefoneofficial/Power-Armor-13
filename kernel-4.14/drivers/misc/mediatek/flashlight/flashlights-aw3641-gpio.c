...
#define AW3641_LEVEL_NUM (2+1)
#define AW3641_HW_TIMEOUT 220

/* define usage count */
static int use_count;

static int aw3641_level = -1;
...
static int aw3641_enable(void)
{
	...
	//aw3641 flash I = Vfb/Rsense *(11-step)*0.1 step:1-8
	if (aw3641_level == 1){
		aw3641_pinctrl_set(HWEN_PIN, 1); //torch mode
	}else if (aw3641_level >= 2){
		aw3641_pinctrl_set(FLASH_PIN, 1);	//flash mode
		for(i = 0;i < flash_level;i++){
			aw3641_pinctrl_set(HWEN_PIN, 0);
			udelay(2); //0.75-10us
			aw3641_pinctrl_set(HWEN_PIN, 1);
			udelay(2); //0.75-10us
		}
	}
	return 0;
...
static int aw3641_set_level(int level)
{
	...
	return 0;//aw3641_pinctrl_set(FLASH_PIN, state);
}
...
static int aw3641_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_GET_DUTY_NUMBER:
		...
		fl_arg->arg = AW3641_LEVEL_NUM;
		break;
	case FLASH_IOC_GET_MAX_TORCH_DUTY:
		pr_debug("FLASH_IOC_GET_MAX_TORCH_DUTY(%d)\n", channel);
		fl_arg->arg = AW3641_LEVEL_NUM - 1;
		break;
	case FLASH_IOC_GET_HW_TIMEOUT:
		pr_debug("FLASH_IOC_GET_HW_TIMEOUT(%d)\n", channel);
		fl_arg->arg = AW3641_HW_TIMEOUT;
		break;
	default:
		...
		struct aw3641_platform_data *pdata)
{
	...
	if (of_property_read_u32(np, "torch_level", &torch_level)){
		pr_info("Parse no dt, torch_level.\n");
		torch_level = 5;//0-10 duty 
	}else{
		if (torch_level > 10){
			torch_level = 10;
		}
	...
	if (of_property_read_u32(np, "flash_level", &flash_level)){
		pr_info("Parse no dt, flash_level.\n");
		flash_level = 1;//1:max 8:min
	}else{
		if (flash_level > 8){
			flash_level = 8;
		}
